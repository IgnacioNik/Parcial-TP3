...
1.  **Registro de Usuario (`/auth/create`):** Un usuario puede crear una cuenta nueva.
2.  **Login de Usuario (`/auth/login`):** Un usuario puede iniciar sesión con sus credenciales.
3.  **Obtención de Datos del Usuario (`/users/{id}`):** La app obtiene y muestra el balance y los datos del header del usuario logueado.
4.  **Obtención de Transacciones (`/transactions`):** La app obtiene la lista de transacciones del usuario, las guarda en la base de datos (Room) y las muestra.
...
* **¿Cómo entrar como invitado?**
    1.  Ve a la pantalla de "Log In".
    2.  **No escribas nada** en los campos de "Email" o "Password". Déjalos vacíos.
    3.  Presiona el botón **"Log In"**.
